...
function AppFormModal({ open, onClose, onSubmit, c, t, defaults, title = 'Log application', submitLabel = 'Add' }) {
  ...
  const { xp: previewXp, gold: previewGold, qs } = useMemo(() =>
    computeRewards({ type, cvTailored, motivation }),
    [type, cvTailored, motivation]
  );
  const focusC = useMemo(() => focusCost(type), [type]);
      ...
      <div ref={boxRef} tabIndex={-1} className="absolute inset-x-0 bottom-0 rounded-t-2xl p-4 outline-none" style={{ background: c.surface }}>
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            <div className="text-[11px] tabular-nums" style={{ color: Grey }}>
              QS {qs} · Focus {focusC} · XP {previewXp} · Gold {previewGold}
            </div>
          </div>
